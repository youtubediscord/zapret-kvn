...
DNS внутри туннеля настраивается автоматически: приватный резолвер из строки `DNS =`,
а если там публичный адрес — резолвер шлюза туннеля (серверы Amnezia перехватывают
порт 53 и отвечают только со своего шлюза).
